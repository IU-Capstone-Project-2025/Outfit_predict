...
def test_qdrant_search_success():
    with patch("app.storage.qdrant_client.QdrantClient") as mock_qdrant:
        mock_client = MagicMock()
        mock_qdrant.return_value = mock_client
        mock_client.search.return_value = [{"id": 1, "score": 0.9}]
        service = QdrantService("url", "api_key")
        result = service.search("collection", [0.1, 0.2])
        assert isinstance(result, list)


def test_qdrant_search_failure():
    with patch("app.storage.qdrant_client.QdrantClient", side_effect=Exception("fail")):
        service = QdrantService("url", "api_key")
        result = service.search("collection", [0.1, 0.2])
        assert result == []
