...
def test_minio_upload_success():
    with patch("app.storage.minio_client.Minio") as mock_minio:
        mock_client = MagicMock()
        mock_minio.return_value = mock_client
        mock_client.fput_object.return_value = True
        service = MinioService("endpoint", "access", "secret", "bucket")
        result = service.upload_file("file", "object")
        assert result is True


def test_minio_upload_failure():
    with patch("app.storage.minio_client.Minio", side_effect=Exception("fail")):
        service = MinioService("endpoint", "access", "secret", "bucket")
        result = service.upload_file("file", "object")
